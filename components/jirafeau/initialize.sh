...
/vagrant/script/guest/create_tor_key.sh ./files/etc/tor/lib/hidden_service/WEBtor/
...
/vagrant/script/guest/add_hidden_service.sh /etc/tor/lib/hidden_service/WEBtor/ 443 127.0.0.1:443

# Create a new TLS certificate for the web server.
# Depends on the Tor service keys being already created.
/vagrant/script/guest/create_ssl_cert.sh ${ONION_HOSTNAME} ./files/etc/ uhttpd
