...
# When using Vagrant.
vagrant)
  vagrant ssh -c "/vagrant/script/guest/menuconfig.sh \"$1\""
  ;;

# When using Docker.
docker)
  # TODO: race condition here. Tor provisioning should be done when starting the container, not each build.
  ssh -oStrictHostKeyChecking=no vagrant@127.0.0.1 -p 22222 -i ./script/builder-keys/ssh.priv /vagrant/script/guest/prov-tor.sh
