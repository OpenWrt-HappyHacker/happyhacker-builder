...
<?php
        if (!empty($crypt_key)){
            echo $cfg['web_root'] . '/f.php?h=' . $link_name . '&amp;d=1' . '&amp;k=' . urlencode($crypt_key);
        }
        else
            echo $cfg['web_root'] . 'dd/' . $link_name . '/'. urlencode($link['file_name']);
?>';
        ...
        if ($cfg['preview'] && jirafeau_is_viewable($link['mime_type']))
        {
            ...
            <?php
            if (!empty($crypt_key)){
                echo $cfg['web_root'] . '/f.php?h=' . $link_name . '&amp;p=1' . '&amp;k=' . urlencode($crypt_key);
            }
            else
                  echo $cfg['web_root'] . 'dd/' . $link_name . '/'. urlencode($link['file_name']);
            ?>';
...
if (!jirafeau_is_viewable ($link['mime_type']) || !$cfg['preview'] || $do_download)
    header ('Content-Disposition: attachment; filename="' . str_replace('"', "'", $link['file_name']) . '"');
else
    header ('Content-Disposition: filename="' . str_replace('"', "'", $link['file_name']) . '"');
