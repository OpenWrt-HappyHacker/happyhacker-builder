...
{
    // Debug parameters
    //window.alert('url: ' + url + ' reference: ' + reference + ' delete_code: ' + delete_code + ' filename: ' + filename + ' date: ' + date + ' crypt_key: ' + crypt_key )
    ...
    if (!!document.getElementById('preview_link'))
    {
        ...
        var preview_link = url + 'dd/' + reference + '/' + filename;
        var preview_link_href = preview_link;

        if (crypt_key.length > 0)
        {
            preview_link = url + 'f.php?h=' + reference + '&amp;p=1' + '&amp;k=' + crypt_key;
            preview_link_href = url + 'f.php?h=' + reference + '&p=1' + '&k=' + crypt_key;
        }
    ...
    var direct_download_link = url + 'dd/' + reference + '/' + filename;
    var direct_download_link_href = direct_download_link;
    if (crypt_key.length > 0)
    {
        direct_download_link =  url + 'f.php?h=' + reference + '&amp;k=' + crypt_key;
        direct_download_link_href = url + 'f.php?h=' + reference + '&k=' + crypt_key;
    }
        ...
        if (req.readyState == 4 && req.status == 200)
        {
            ...
            if (time != 'none')
            {
                ...
                if(!add_time_string_to_date(d, time))
                    return;
                show_link (url, res[0], res[1], res[2], res[3], d.toString());
            }
            else
                show_link (url, res[0], res[1], res[2], res[3]);
        }
    }
        ...
        if (req.readyState == 4 && req.status == 200)
        {
            ...
            if (async_global_time != 'none')
            {
                ...
                if(!add_time_string_to_date(d, async_global_time))
                    return;
                show_link (async_global_url, res[0], res[1], res[2], res[3], d.toString());
            }
            else
                show_link (async_global_url, res[0], res[1], res[2], res[3]);
        }
    }
