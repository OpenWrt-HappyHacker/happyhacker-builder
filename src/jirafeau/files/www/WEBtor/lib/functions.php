...
/**
 * Patch to support unix time timestamp witout full time zones support
...
function
jirafeau_delete_link ($md5_link)
{
    $l = jirafeau_get_link ($md5_link);
    if (!count ($l))
        return;

    jirafeau_clean_rm_link ($md5_link);

    $md5 = $l['md5'];
    $p = s2p ("$md5");
    
    $dirpath_symlink = VAR_STATIC . $md5_link . '/';
    
    // Removes static resource  
    if (file_exists ($dirpath_symlink))
        @deleteDirectory ($dirpath_symlink);
...
jirafeau_upload ($file, $one_time_download, $key, $time, $ip, $crypt, $link_name_length)
{
    ...
    fwrite ($handle,
            ...
            NL . $md5. NL . ($one_time_download ? 'O' : 'R') . NL . date_U () .
...
jirafeau_async_end ($ref, $code, $crypt, $link_name_length)
{
    ...
    $size = filesize($p);
    $np = s2p ($md5);
    $delete_link_code = jirafeau_gen_random (10);
    ...
    /* Create link. */
    // TODO: Check security, random links mandatory
    $link_tmp_name =  VAR_LINKS . $md5 . rand (0, 10000) . '.tmp';
    $handle = fopen ($link_tmp_name, 'w');
    // WORKAROUND: Random entropy added to file content at end,
    // poor's solution seal approved, Mario ?
    fwrite ($handle,
            $a['file_name'] . NL . $a['mime_type'] . NL . $size . NL .
            $a['key'] . NL . $a['time'] . NL . $md5 . NL . $a['onetime'] . NL .
            date_U () . NL . $a['ip'] . NL . $delete_link_code . NL . ($crypted ? 'C' : 'O') . NL . rand (0,1000000));
    fclose ($handle);
    $md5_link = substr(base_16_to_64 (md5_file ($link_tmp_name)), 0, $link_name_length);
    $l = s2p ("$md5_link");
    if (!@mkdir (VAR_LINKS . $l, 0755, true) ||
    ...
    $filename_ucode = urlencode($a['file_name']);
    $filepath_symlink = VAR_STATIC . $md5_link . '/'. $filename_ucode;
    $filepath_datastore = VAR_FILES . $np . $md5;
    $dirpath_public_dd =  dirname(dirname(__FILE__)) . '/dd';
    
    // Static resource exists ?, if not create it. 
    if (!file_exists (dirname($filepath_symlink)))
        @mkdir (dirname($filepath_symlink), 0755, true);
    if (!file_exists ($filepath_symlink))
        @symlink ($filepath_datastore,$filepath_symlink);
        
    // Static resources are public in direct download directory ? , if not create it. 
    if (!file_exists ($dirpath_public_dd))
        @symlink (VAR_STATIC,$dirpath_public_dd);
        
   // trigger_error( 'filepath_symlink: ' .  $filepath_symlink . NL. ' dirname: '.dirname($filepath_symlink), E_USER_WARNING);
    
    /* Clean async upload. */
    jirafeau_async_delete ($ref);
    //function show_link (url, reference, delete_code, filename, crypt_key, date)
    return $md5_link . NL . $delete_link_code . NL . $filename_ucode  . NL . urlencode($crypt_key);
}
